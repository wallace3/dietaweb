      ...
      <b-row>
        <UIComponentCard id="basic" title="Tabla de wishlist">
            <b-table-simple responsive class="table-centered mb-0">
                ...
                <b-tbody>
                    <b-tr  v-for="item in filteredWishlist" :key="item.idWishlist">
...
const filteredWishlist = computed(() => {
  ...
  let filtered = wishlist.value.filter(category => {
...
const paginatedWishlist = computed(() => {
  ...
  return filteredWishlist.value.slice(start, end);
});

// Calcular el número total de páginas
const totalPages = computed(() => {
  return Math.ceil(filteredWishlist.value.length / itemsPerPage.value);
});
