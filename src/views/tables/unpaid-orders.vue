<template>
    <div>
      <!-- Tabla de ordenes -->
                ...
                <b-table-simple responsive class="table-centered mb-0">
                    ...
                    <b-tbody>
                        <b-tr  v-for="order in filteredorders" :key="order.idOrder">
                            ...
                            <b-td>{{ order.name }} {{ order.lastName }}</b-td>
        ...
        <b-modal v-model="editModal" title="Detalle de orden" size="xl"  cancel-title="Cancelar" centered>
            ...
            <b-table-simple responsive hover class="table-centered mb-0">
              <b-thead variant="light">
                <b-tr>
                  ...
                  <b-th scope="col">Descripción</b-th>
                  <b-th scope="col">Previo de venta</b-th>
                </b-tr>
              </b-thead>
              <b-tbody>
                <b-tr v-for="det in details" :key="det.idDetailOrder">
                  <b-td>
                    <div class="d-flex align-items-center gap-1">
                      <img :src="'http://localhost:8080/' + det.image_url" alt="" class="avatar-sm rounded-circle"/>
                      <div class="d-block">
                        <h5 class="mb-0">{{ det.name }}</h5>
                      </div>
                    </div>
                  </b-td>
                  <b-td>{{ det.description }}</b-td>
                  <b-td>{{ det.price }}</b-td>
                </b-tr>
              </b-tbody>
            </b-table-simple>
...
<script setup>
    ...
    const editOrder = async(id) => {
        try{
            const response = await fetch(`http://localhost:8080/stripe/orderDetails/${id}`);
            if(!response.ok){
                throw new Error(`Error HTTP: ${response.status}`);
            }
            const data = await response.json();
