...
const getResults = async () => {
    try {
        const response =  await fetch(`http://localhost:8080/auction/underbidders/${idAuction.value}`);
        if (!response.ok) {
...
const filteredResults = computed(() => {
    let filtered = results.value.filter(result => {
        return result.auction.toLowerCase().includes(searchQuery.value.toLowerCase());
    });
    if (sortKey.value) {
